...
app.get('/profile', function (req, res) {
    if (req.cookies.readit_auth != undefined) {
        connection.query('SELECT * FROM auth_tokens WHERE token = ? AND expired = 0', [req.cookies.readit_auth], function (err, token, fields) {
            if (token.length > 0) {
                if (token) {
                    connection.query('SELECT * FROM users WHERE id = ?', [token[0].id], function (err, user, fields) {
                        connection.query('SELECT COUNT(id) as subs from subscriptions where userID = ?', [token[0].id], function (err, subscriptions, fields) {
                            connection.query('SELECT COUNT(id) as postCount from posts where creatorID = ?', [token[0].id], function (err, posts, fields) {
                                connection.query('SELECT * FROM categories', function (err, a_categories, fields) {
